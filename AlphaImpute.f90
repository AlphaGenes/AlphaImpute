...
if (trim(TempOpt)=="Yes") then
	...
	HetGameticStatus=9
	if (trim(TempHetGameticStatus)=="Male") then
		HetGameticStatus=1
		HomGameticStatus=2
	endif	
	if (trim(TempHetGameticStatus)=="Female") then
		HetGameticStatus=2
		HomGameticStatus=1
	endif	
...
nProcessAlphaPhase=nProcessors-nProcessGeneProb
...
enddo
rewind(47)
nAnisPrePhased=nAnisPrePhased/2

CountPrePhased=0
do k=1,nAnisPrePhased
	read (47,*) dumC,WorkPhase(:,1)
	read (47,*) dumC,WorkPhase(:,2)
	do i=1,nAnisP
...
allocate(PhaseHD(nAnisHD,nSnp,2,2))
...
call ParentHomoFill
call PhaseComplement
call ImputeParentByProgenyComplement
call MakeGenotype
...
do i=1,nAnisP
	if ((SexOpt==0).or.((SexOpt==1).and.(RecGender(i)/=HetGameticStatus))) then  
		do e=1,2
			ParId=RecPed(i,e+1)
			do j=1,nSnp
				if (ImputePhase(i,j,e)==9) then
					if ((ImputePhase(ParId,j,1)==ImputePhase(ParId,j,2)).and.(ImputePhase(ParId,j,1)/=9)) ImputePhase(i,j,e)=ImputePhase(ParId,j,1)
				endif
			enddo
		enddo
	else
		...
		do j=1,nSnp
			if (ImputePhase(i,j,1)==9) then		!Comment from John Hickey see analogous iterate subroutine
				if ((ImputePhase(ParId,j,1)==ImputePhase(ParId,j,2)).and.(ImputePhase(ParId,j,1)/=9)) ImputePhase(i,j,:)=ImputePhase(ParId,j,1)
			endif
		enddo
	endif
enddo
...
do i=1,nAnisP
	...
	j=0
	do m=1,nSnpRaw
		if (SnpIncluded(m)==1) then
			j=j+1
			if (ImputeGenos(i,j)==1) then
				if (ImputeGenos(RecPed(i,2),j)==1) then	 
					if ((ImputePhase(i,j,1)==0).or.(ImputePhase(i,j,1)==1)) then
						Informativeness(j,1)=1
						GlobalTmpCountInf(i,1)=GlobalTmpCountInf(i,1)+1
						TmpInfor(GlobalTmpCountInf(i,1),1)=j
					endif				
				endif
	...
	enddo
	GlobalTmpCountInf(i,7)=count(ImputePhase(i,:,1)/=9)
	GlobalTmpCountInf(i,8)=count(ImputePhase(i,:,2)/=9)	
	if (GlobalTmpCountInf(i,1)>0) MSTermInfo(i,1)=1
	if (GlobalTmpCountInf(i,2)>0) MSTermInfo(i,2)=1	
	do k=1,6
...
do i=1,nAnisP
	...
	do j=1,nAnisP
		if ((RecPed(j,2)==i).or.(RecPed(j,3)==i)) then
			...
			do k=1,nSnp
				if (TempGenos(i,k)==9) then
					if (TempGenos(j,k)==0) Count0(k)=Count0(k)+1
					if (TempGenos(j,k)==1) Count1(k)=Count1(k)+1
					if (TempGenos(j,k)==2) Count2(k)=Count2(k)+1
				endif
			enddo
		endif
	enddo
	...
	do k=1,nSnp
		if ((Count0(k)+Count1(k)+Count2(k))>OffspringFillMin) then
			if (Count0(k)==0) TempGenos(i,k)=2
			if (Count2(k)==0) TempGenos(i,k)=0
			if ((SexOpt==1).and.(RecGender(i)==HetGameticStatus)) cycle
			if ((Count0(k)>2).and.(Count2(k)>2)) TempGenos(i,k)=1  
		endif
	enddo
enddo
...
do e=1,2
	...
	do i=1,nAnisRawPedigree
		...
		if ((IndId/=0).and.(ParId/=0).and.(TurnOn==1)) then
			...
			enddo
			if ((float(CountDisagree)/CountBothGeno)>DisagreeThreshold) then
				Ped(i,ParPos)='0'
				write (101,*) Ped(i,1)
