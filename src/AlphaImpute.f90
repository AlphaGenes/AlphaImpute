...
interface 
  subroutine readinparameterfile(specfile, inputin)
    use alphaimputeinmod
...
subroutine ReadInParameterFile(SpecFile ,inputIn)
    ...
    READFILE: do while (IOStatus==0)
        ...
        if (first(1:1)=="=" .or. len(trim(line))==0) then
          cycle
            ...
            case("truegenotypefile")
                 if (.not. allocated(second)) then
                    ...
                    if (input%TrueGenotypeFile=="None") then
                        input%TrueGenos1None0=0
                    else
                        input%TrueGenos1None0=1
                    endif
                endif
            ...
            case("numbersnp")
                read(second(1),*) input%nsnp
                if (input%nsnp>240000) then
            ...
            case("multiplehdpanels")
                ...
                allocate(input%nSnpByChip(input%MultiHD))
                
            case("numbersnpxchip")
                do i=1,input%MultiHD
                     read(second(i),*) input%nSnpByChip(i)
                enddo
            case("hdanimalsthreshold")
                read(second(1),*) input%PercGenoForHD
            ...
            case("editingparameters")
                ...
                if (input%IntEditStat==1) then
                    ...
                    read(second(1),*) input%PercGenoForHD
                    read(second(2),*) input%PercSnpMiss
                    read(second(3),*) input%SecondPercGenoForHD
            ...
            case("numberphasingruns")
                ...
                if (ToLower(trim(second(1))) == "phasedone") then
                    ...
                    read(second(3),*) input%nPhaseInternal
                else if(ToLower(trim(second(1))) == "nophase") then
                    input%noPhasing = 1
                    input%managephaseon1off0 = 0
                else
                    input%managephaseon1off0 = 1
                    read(second(1),*) input%nPhaseExternal 
            ...
            case("coreandtaillengths")
                ...
                if (size(second) /= input%nPhaseExternal) then
                ...
                do i=1,size(second)
                    read(second(i), *) input%CoreAndTailLengths(i)
                enddo
            ...
            case("corelengths")
                ...
                if (size(second) /= size(input%corelengths)) then
                    write(error_unit,*) "Error: numberofphasingruns is set to a different number of parameters than what is specified here. \n Please set this to the same number of parameters that are given for CoreAndTailLengths and CoreLengths"
                    stop 40532
                endif
                do i=1,size(second)
                    read(second(i),*) input%corelengths(i)
                enddo
            ...
            case("genotypeerror")
                if (input%nPhaseExternal /= 0) then
                    read(second(1), *) input%GenotypeErrorPhase
                endif

            case("numberofprocessorsavailable")
                read(second(1),*) input%nProcessors

            case("largedatasets")
                if (ToLower(trim(second(1)))== "yes") then
                    input%largedatasets=.true.
                    read(second(2),*) input%PhaseSubsetSize
                    read(second(3),*) input%PhaseNIterations
                else 
            ...
            case("internaliterations")
                read(second(1), *) input%InternalIterations
            ...
            case("wellphasedthreshold")
                read(second(1),*) input%WellPhasedThresh

            case("input%hmmoption")
                ...
                if (input%hmmoption==RUN_HMM_NULL) then
                    write(error_unit,*), "input%hmmoption not correctly specified"
                    stop
                endif
            ...
            case("templatehaplotypes")
                ...
                read(second(1), *) input%nHapInSubH

            case("burninrounds")
                read(second(1), *) input%HmmBurnInRound
            case("rounds")
                read(second(1), *)input%nRoundsHMM
            case("parallelprocessors")
                read(second(1), *) input%useProcs
            case("seed")
                read(second(1), *)input%idum
            case("thresholdmissingalles")
                read(second(1), *) input%phasedThreshold
            case("thresholdimputed")
                read(second(1), *) input%imputedThreshold
            ...
            case("userdefinedalphaphaseanimalsfile")
                ...
                if (second(1)/="None") then
                    ...
                    open (newunit=input%AnimalFileUnit,file=trim(second(1)),status="old")
                endif
            ...
            case("prephasedfile")
                if (second(1)=="None") then
                    ...
                    input%PrePhased=1
                    open (newunit=input%prePhasedFileUnit,file=trim(second(1)),status="old")
                endif
            case("input%bypassgeneprob")
                if (trim(second(1))=="No") then
                    input%BypassGeneProb=0             
                else if (trim(second(1))=="Yes") then
                    input%BypassGeneProb=1
            ...
            case("input%restartOption")
                read(second(1),*) input%restartOption
    ...
    input%nProcessAlphaPhase=input%nProcessors-input%nProcessGeneProb ! Never used!
...
!$  CALL OMP_SET_NUM_THREADS(input%nProcessors)
end subroutine ReadInParameterFile
